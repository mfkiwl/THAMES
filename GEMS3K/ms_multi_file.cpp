...
void TMulti::out_multi( GemDataStream& ff  )
{
     ...
     arr[0] = paTProfil->p.PC;
     arr[1] = paTProfil->p.PD;
     arr[2] = paTProfil->p.PRD;
     arr[3] = paTProfil->p.PSM;
     arr[4] = paTProfil->p.DP;
     arr[5] = paTProfil->p.DW;
     arr[6] = paTProfil->p.DT;
     arr[7] = paTProfil->p.PLLG;
     arr[8] = paTProfil->p.PE;
     arr[9] = paTProfil->p.IIM;

   ff.writeArray( arr, 10 );
   ff.writeArray( &paTProfil->p.DG, 28 );
   to_file( ff );
}
...
void TMulti::read_multi( GemDataStream& ff, DATACH  *dCH )
{
    ...
    ff.readArray( arr, 10 );
    paTProfil->p.PC = arr[0];
    paTProfil->p.PD = arr[1];
    paTProfil->p.PRD = arr[2];
    paTProfil->p.PSM = arr[3];
    paTProfil->p.DP = arr[4];
    paTProfil->p.DW = arr[5];
    paTProfil->p.DT = arr[6];
    paTProfil->p.PLLG = arr[7];
    paTProfil->p.PE = arr[8];
    paTProfil->p.IIM = arr[9];

    ff.readArray( &paTProfil->p.DG, 28 );
    from_file( ff );
