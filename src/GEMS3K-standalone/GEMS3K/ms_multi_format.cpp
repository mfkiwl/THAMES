                                  ...
                                  bool with_comments, bool brief_mode )
{
    ...
    if( !brief_mode || !essentiallyEqual(pa->p.DB, pa_.p.DB ) )
        prar.writeField(f_pa_DB, pa->p.DB, _comment, false  );
    if( !brief_mode || !essentiallyEqual(pa->p.DHB, pa_.p.DHB ) )
        prar.writeField(f_pa_DHB, pa->p.DHB, _comment, false  );
    if( !brief_mode || !essentiallyEqual(pa->p.EPS, pa_.p.EPS ) )
        prar.writeField(f_pa_EPS, pa->p.EPS, _comment, false  );
    if( !brief_mode || !essentiallyEqual(pa->p.DK, pa_.p.DK ) )
        prar.writeField(f_pa_DK, pa->p.DK, _comment, false  );
    if( !brief_mode || !essentiallyEqual(pa->p.DS, pa_.p.DS ) )
        prar.writeField(f_pa_DS,  pa->p.DS, _comment, false  );
    if( !brief_mode || !essentiallyEqual(pa->p.DF, pa_.p.DF ) )
        prar.writeField(f_pa_DF, pa->p.DF, _comment, false  );
    if( !brief_mode || !essentiallyEqual(pa->p.DFM, pa_.p.DFM ) )
    ...
    if(!brief_mode || !essentiallyEqual(pa->p.AG, pa_.p.AG ) )
        prar.writeField(f_pa_AG,  pa->p.AG, _comment, false  );
    if(!brief_mode || !essentiallyEqual(pa->p.DGC, pa_.p.DGC ) )
    ...
    if(!brief_mode || !essentiallyEqual(pa->p.GAR, pa_.p.GAR ) )
        prar.writeField(f_pa_GAR,  pa->p.GAR, _comment, false  );
    if(!brief_mode || !essentiallyEqual(pa->p.GAH, pa_.p.GAH ) )
    ...
    if(!brief_mode || !essentiallyEqual(pa->p.XwMin, pa_.p.XwMin ) )
        prar.writeField(f_pa_XwMin,  pa->p.XwMin, _comment, false  );
    if(!brief_mode || !essentiallyEqual(pa->p.ScMin, pa_.p.ScMin ) )
        prar.writeField(f_pa_ScMin,  pa->p.ScMin, _comment, false  );
    if(!brief_mode || !essentiallyEqual(pa->p.DcMin, pa_.p.DcMin ) )
        prar.writeField(f_pa_DcMin,  pa->p.DcMin, _comment, false  );
    if(!brief_mode || !essentiallyEqual(pa->p.PhMin, pa_.p.PhMin ) )
        prar.writeField(f_pa_PhMin,  pa->p.PhMin, _comment, false  );
    if(!brief_mode || !essentiallyEqual(pa->p.ICmin, pa_.p.ICmin ))
    ...
    if(!brief_mode || !essentiallyEqual(pa->p.DFYw, pa_.p.DFYw ) )
        prar.writeField(f_pa_DFYw,  pa->p.DFYw, _comment, false  );
    if(!brief_mode || !essentiallyEqual(pa->p.DFYaq, pa_.p.DFYaq) )
        prar.writeField(f_pa_DFYaq,  pa->p.DFYaq, _comment, false  );
    if(!brief_mode || !essentiallyEqual(pa->p.DFYid, pa_.p.DFYid ) )
        prar.writeField(f_pa_DFYid,  pa->p.DFYid, _comment, false  );
    if(!brief_mode || !essentiallyEqual(pa->p.DFYr, pa_.p.DFYr ) )
        prar.writeField(f_pa_DFYr,  pa->p.DFYr, _comment, false  );
    if(!brief_mode || !essentiallyEqual(pa->p.DFYh, pa_.p.DFYh ) )
        prar.writeField(f_pa_DFYh,  pa->p.DFYh, _comment, false  );
    if(!brief_mode || !essentiallyEqual(pa->p.DFYc, pa_.p.DFYc ) )
        prar.writeField(f_pa_DFYc,  pa->p.DFYc, _comment, false  );
    if(!brief_mode || !essentiallyEqual(pa->p.DFYs, pa_.p.DFYs ) )
    ...
    if(!brief_mode || !essentiallyEqual(pa->p.GAS, pa_.p.GAS ) )
        prar.writeField(f_pa_GAS,  pa->p.GAS, _comment, false  );
    if(!brief_mode || !essentiallyEqual(pa->p.DG, pa_.p.DG ) )
        prar.writeField(f_pa_DG,  pa->p.DG, _comment, false  );
    if(!brief_mode || !essentiallyEqual(pa->p.DNS, pa_.p.DNS ) )
        prar.writeField(f_pa_DNS, pa->p.DNS, _comment, false  );
    if(!brief_mode || !essentiallyEqual(pa->p.IEPS, pa_.p.IEPS) )
        prar.writeField(f_pa_IEPS, pa->p.IEPS, _comment, false  );
    prar.writeField(f_pKin, pm.PLIM, _comment, brief_mode  );
    if(!brief_mode || !essentiallyEqual(pa->p.DKIN, pa_.p.DKIN ) )
