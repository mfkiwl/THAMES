...
struct SorptionSiteData {
  ...
  long int NSpec_; // Number of surface species that can bind to this site (>=1;
                   // 0 if the site ignored)
  ...
  double *arDent;  // Species denticity or coordination number on this site type
                   // [NSpecies]
  double *arSpDUL; // temporary upper constraint on species amount for SAT
                   // calculations [NSpecies]
  double *ar_nxs;  // moles of sites taken by a surface species [NSpecies]
  long int
      *ar_xst; // Index of surface species on surface tile (phase)  [NSpecies]
  double *arlnSACT; // ln of SACT [NSpecies] - output (direct access,
                    // incremental) ?????
  double *arlnGamF; // ln of Frumkin or BET term [NSpecies] - output (direct
                    // access, incremental) ????
...
protected:
  ...
  long int NSpec; // Number of surface species on this site (at least 1; 0 if
                  // site to be ignored)
  ...
  // double MASDJ[DFCN]; Parameters of surface species in surface complexation
  // models enum {
  //	//[0] - max site density in mkmol/(g sorbent); [1] - species charge
  // allocated to 0 plane;
  //	//[2] - surface species charge allocated to beta -or third plane; [3] -
  // Frumkin interaction parameter;
  //	//[4] species denticity or coordination number; [5]  - reserved
  // parameter (e.g. species charge on 3rd EIL plane)]
  //   XL_ST = 0, XL_EM, XL_SI, XL_SP
  // };

  double *Dent;  //  Species denticity or coordination number [NSpec]
  double *SpDUL; // temporary upper constraint on species amount for SAT
                 // calculations [NSpec]
  ...
  long int *xst; // Index of surface species on surface tile (phase)  [NSpec]
                 // results
  //   double (*D)[MST];  // Reserved; new work array for calc. surface
  //   act.coeff.
  // double lnSAC[4];   // former lnSAT ln surface activity coeff and Coulomb's
  // term  [Lads][4]
  double *ISAT;   // ISAT for each species (in SACT calculations) [NSpec]
  double *eF;     // Frumkin exponent acting on species [NSpec]
  double *SACT;   // Surface activity coefficient terms (config. entropy terms)
                  // [NSpec]
  double *lnSACT; // ln of SACT on this site type [NSpec] - output
  double *
      lnGamF; // ln activity coefficient due to Frumkin or BET isotherm - output
...
struct SorptionData {
  ...
  char
      EIL_Code_; // Code for specific EIL model- see EILMODCODES  (before: SCMC)
  char PhasNam_[MAXPHASENAME]; // Phase name (for specific built-in models)

  long int NSpec_; // Total number of species assigned to this surface tile or
                   // Donnan phase
  long int
      nlPh_; // number of linked phases (cf. lPh), default 1 (the sorbent phase)
  long int nlPhC_; // number of linked phase parameter coefficient per link
                   // (default 0)

  long int
      nEIml_; // number of EIL model layers (default 0, maximum MAXEILAYERS=4);
  ...
  long int NsiteTs_; // Number of surface site types per this surface patch (min
                     // 1 max 6), for Donnan 1
  ...
  long int kSorPh_; // Index of the sorbent phase in GEM IPM work structure
                    // (MULTI) if -1 then this is a site-balance based approach;
                    // SSa_ or sVp_ must be provided here

  long int *xsMd_; // denticity of surface species per surface site (site
                   // allocation) [NSpec][NsiteTs]

  // long int *arPsDiS_;  // array of DC denticity and indexes of binding sites
  // [NSpec*(DelMax+1)]

  char *DC_Cods_; // DC class codes for species [NSpec_]
  char *IsoCt_;   // isotherm and SATC codes for surface site types [2*NsiteTs_]

  long int *arPhLin_; // indexes of linked (sorbent) phase(s) and link type
                      // code(s) [nlPh*2] read-only
  ...
  //           sgw_, // Standard mean surface energy of solid-aqueous interface,
  //           J/m2 sgg_, // Standard mean surface energy of gas-aqueous
  //           interface, J/m2 rX0_,    // Mean radius r0 for (spherical or
  //           cylindrical) particles, nm (reserved) hX0_,    // Mean thickness
  //           h0 for cylindrical or 0 for spherical particles, nm (reserved)
  //
  ...
  //
  //           *nPul_, // pointer to upper restriction to this phase amount, mol
  //           (calculated here) *nPll_, // pointer to lower restriction to this
  //           phase amount, mol (calculated here) *sGP_;  // pointer to surface
  //           free energy of the phase, J (YOF*PhM)

  double N_Cst_; // Standard surface number density, 1/nm2
  double G_Cst_; // Standard surface density, mol/m2
  double
      q_Cst_; // Standard sorption capacity, mol/kg(sorbent) or eq/kg(sorbent)

  double Nfsp_, // Fraction of the sorbent specific surface area or volume
                // allocated to surface type (>0 <10000)
      MASDT_, // Total sorption capacity for this surface type (mol/kg), before
              // was (mkmol/g)
  ...
  double *
      IsoSc; // Isotherm equation coeffs per surface site type [NsiteTs*nISTcf_]

  double *arPparc; // Partial pressures -> NSpecies
  double *arWx;    // Species mole fractions ->NSpec_ read-only
  double *arnx;    // Pointer to mole amounts of phase components (provided)
                   // [NSpec_] read-only
  // output
  double *arlnScalT;  // Surface/volume scaling activity correction terms
                      // [NSpec_]  direct access
  double *arlnSACT;   // Pointer to ln SACT for surface species [NSpec_]  direct
                      // access
  double *arlnGammaF; // Pointer to Frumkin or BET non-electrostatic activity
                      // coefficients [NSpec_] direct access
  double *arCTerms;   // Pointer to Coulombic correction terms (electrostatic
                      // activity coefficients) [NSpec_] direct access
  double *arlnGamma;  // Pointer to ln activity coefficients of sorption phase
                      // components mixing [NSpec_] direct access (memory under
                      // pointers must be provided from the calling program)

  double *arVol; // molar volumes of end-members (species) cm3/mol [NSpec_]
                 // read-inly
  char (*arSM)[MAXDCNAME]; // pointer to the list of DC names in the phase
                           // [NSpec_] read-only
  char *arDCC; // pointer to the classifier of DCs involved in this phase
               // [NSpec_] read-only
};

class TSorpMod { // Treatment of surface tile (patch) or Donnan volume phases in
                 // sorption models
protected:
  ...
  long int NSpec; // Total number of species assigned to this surface tile or
                  // Donnan phase
  long int
      nlPh; // number of linked phases (cf. lPh), default 1 (the sorbent phase)
  long int nlPhC; // number of linked phase parameter coefficient per link
                  // (default 0)
  ...
  long int NsiteTs; // Number of surface site types per surface patch type (min
                    // 1 max 6), for Donnan 1
                    //   (if 0 then this sorption phase model is ignored)
  ...
  long int kSorPh; // Index of the sorbent phase in GEM IPM work structure
                   // (MULTI) if -1 then this is a site-balance based approach;
                   // SSa_ or sVp_ must be provided

  long int **xsMd; // denticity of surface species per surface site (site
                   // allocation) [NSpec][NsiteTs]
  //    long int **PsDS;    // array of DC denticity and indexes of binding
  //    sites [NSpec][DelMax+1]
  long int (*PhLin)[2]; // indexes of linked (sorbent) phase(s) and link type
                        // code(s) [nlPh][2] read-only
  char *DCCs;           // DC class codes for species [NSpec_]
  char (
      *IsoCt)[2]; // isotherm and SATC codes for surface site types NsiteTs][2]
  ...
  double sSA, // Specific surface area, m2/g, default: 0.
      ...
      //           rX0,    // Mean radius r0 for (spherical or cylindrical)
      //           particles, nm (reserved) hX0,    // Mean thickness h0 for
      //           cylindrical or 0 for spherical particles, nm (reserved)
      //
  ...
  // model parameters
  double Nfsp, // Fraction of the sorbent specific surface area or volume
               // allocated to surface type (>0 <10000)
      MASDT,   // Total sorption capacity for this surface type (mol/kg), before
               // was (mkmol/g)
      XetaC,   // Total permanent charge capacity CEC, mol/kg
      VetaP,   // Total permanent volume charge density (eq/m3)
      XetaP,   // Total density of surface permanent charge (eq/m2), before
               // mkeq/m2
  ...
  double (*CD)[MAXEILAYERS]; // Species charges allocated to 0, 1 and 2 planes
                             // [NspT]
  ...
  double XsTs, // Total number of moles of species in this tile or Donnan volume
      ...
      Xeta[MAXEILAYERS], // Total charge of surface species on EIL planes 0,1,2,
                         // ..., equiv
  ...
  double *nx; // pointer to moles of surface species on this surface tile
              // (read-only) [Nspec]

  double **lnSACTs; // Work array of ln SACT for surface species on sites
                    // [Nspec][NsiteTs]
  double *XTS[2];   // Total number of moles of surface DC and 'solvent' DC at
                    // surface site [2][NsiteTs]

  // current results
  double Gex, Hex, Sex, CPex, Vex, Aex,
      Uex; // molar electrostatic excess properties for surface species
  //    double Gid, Hid, Sid, CPid, Vid, Aid, Uid;   // molar ideal mixing
  //    properties for surface species
  ...
  double *lnGammaF; // Pointer to Frumkin or BET non-electrostatic activity
                    // coefficients [Nspec]
  double *CTerms;   // Pointer to Coulombic correction terms (electrostatic
                    // activity coefficients) [Nspec]
  double *lnGamma;  // Pointer to ln activity coefficients of sorption phase
                    // components mixing [Nspec] (memory under pointers must be
                    // provided from the calling program)
...
public:
  ...
  virtual long int SorptionSpecies() { return 0; }

  virtual long int PTparam() { return 0; }

  virtual long int IsothermMod() { return 0; }

  virtual long int ElstatMod() { return 0; }
  /*
...
class TNEMcalc : public TSorpMod // Non-electrostatic sorption phase model
...
protected:
  double Xcond, // conductivity of phase carrier, sm/m2, reserved
      ...
      Sigg, // Specific surface free energy for phase-gas interface (J/m2) (not
            // yet used)
      Xr0,  // Mean radius r0 for (spherical or cylindrical) particles, nm
            // (reserved) Xr0h0[2]
      Xh0; // Mean thickness h0 for cylindrical or 0 for spherical particles, nm
           // (reserved)

  //        TSurfPatchMod* patchMod[]; // Pointer to array of TSurfPatch
  //        instances - size NsurT
...
public:
  ...
  TNEMcalc(long int NSpecies, long int NSurSpecies, long int NSorbentEMs,
           long int NSurfTypes, char Mod_Code, char *Phase_Name, char **SM3_,
           char *DCC_, char **SCM_, char **SATT_, double Aalp_, double *arnx_,
           double *arlnGam, double *arlnSACT, double T_k, double P_bar);
  // Destructor
  ~TNEMcalc();
