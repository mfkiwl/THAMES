...
#endif
/// Calculation of LPP-based automatic initial approximation of the primal
/// vector x. Use the modified simplex method with two-side constraints on x
//
void TMulti::AutoInitialApproximation() {
  ...
  try { // Allocation of work arrays
    ...
    B1 = new double[pm.N];
    ErrorIf(!DN || !DU || !B1, "AutoInitialApproximation()",
            "Memory alloc error.");
    for (i = 0; i < pm.N; i++)
      DU[i + Q] = 0.;
    ...
    for (j = 0; j < pm.N; j++)
      B1[j] = pm.B[j];
    k = -1;
    for (i = 0; i < pm.L; i++) { // Loading non-zero values
      if (fabs(pm.G[i]) > 1E-19) {
        k++;
        AA1[k] = -pm.G[i];
        NMB[i] = k + 1;
...
///  SPOS function
//
void TMulti::SPOS(double *P, long int STR[], long int NMB[], long int J,
                  long int M, double AA1[]) {
  long int I, K;
  K = 0;
  for (I = 0; I <= M; I++) {
    if (I == STR[NMB[J] + K - 1]) {
...
///  START function
//
void TMulti::START(long int T, long int *ITER, long int M, long int N,
                   long int NMB[], double GZ, double EPS, long int STR[],
                   long int *BASE, double B[], double UND[], double UP[],
                   double AA1[], double *A, double *Q) {
  long int I, J;
  ...
  for (J = 0; J < N; J++) {
    if (fabs(UP[J]) < EPS)
      ...
      if (fabs(UP[J]) < EPS)
        ...
        Error("E00IPM: SolveSimplex()",
              "Inconsistent LP problem (negative UP[J] value(s) in START()) ");
    }
...
///  NEW function
//
void TMulti::NEW(long int *OPT, long int N, long int M, double EPS,
                 double *LEVEL, long int *J0, long int *Z, long int STR[],
                 long int NMB[], double UP[], double AA1[], double *A) {
  long int I, J, J1;
  double MAX, A1;
  double *P;
  P = new double[M + 1];
...
///  WORK function
//
void TMulti::WORK(double GZ, double EPS, long int *I0, long int *J0,
                  long int *Z, long int *ITER, long int M, long int STR[],
                  long int NMB[], double AA1[], long int BASE[], long int *UNO,
                  double UP[], double *A, double Q[]) {
  double MIM, A1;
  long int UN, J, I;
  double *P;
  P = new double[M + 1];
...
///  FIN function
//
void TMulti::FIN(double EPS, long int M, long int N, long int STR[],
                 long int NMB[], long int BASE[], double UND[], double UP[],
                 double U[], double AA1[], double *A, double Q[],
                 long int * /*ITER*/) {
  long int /* K,*/ I, J;
  double *P;
  P = new double[M + 1];
...
//
void TMulti::SolveSimplex(long int M, long int N, long int T, double GZ,
                          double EPS, double *UND, double *UP, double *B,
                          double *U, double *AA1, long int *STR,
                          long int *NMB) {
  ...
  try {
    ...
    for (i = 0; i < IT; i++) // while(1) fixed  03.11.00
    {
      ...
      WORK(GZ, EPS, &I0, &J0, &Z, &ITER, M, STR, NMB, AA1, BASE, &UNO, UP, A,
           Q);
      if (UNO)
  ...
  FINISH:
    FIN(EPS, M, N, STR, NMB, BASE, UND, UP, U, AA1, A, Q, &ITER);
    delete[] A;
    delete[] Q;
    delete[] BASE;
  } catch (TError &xcpt) {
...
//
double TMulti::CalculateEquilibriumState(long int &NumIterFIA,
                                         long int &NumIterIPM) {
  // const char *key;
  double ScFact = 1.;
  ...
  // cout << "kMM: " << pm.pKMM << "  ITau: " << pm.ITau << "  kTau: " <<
  // pm.kTau << "  kdT: " << pm.kdT << endl;
  if (pm.pKMM < 2) {
    if (pm.ITau < 0 || pm.pKMM != 1) {
      /*  KMretCode = */ CalculateKinMet(
          LINK_TP_MODE); // Re-create TKinMet class instances
      pm.ITau = 0;
      pm.pKMM = 1;
      /*  KMretCode = */ CalculateKinMet(
          LINK_IN_MODE); // Initial state calculation of rates
    }
    //    if( pm.ITau == 0 )
    //    {
    //        KMretCode = CalculateKinMet( LINK_IN_MODE ); // Initial state
    //        calculation of rates
    //    }
    else if (pm.ITau >= 0) {
      /*  KMretCode = */ CalculateKinMet(
          LINK_PP_MODE); // Calculation of rates and metast.constraints at time
                         // step
    }
    //  switch(KMretCode)
    //  {
  ...
  try {
    ...
    case A_TV_: {
      ...
      /*
        GoldenSection gsData( pm.Pai[0], pm.Pai[1], pm.Pai[3], pm.Fdev1[1],
        A_P); pm.P = gsData.getMinimum(); A_P(pm.P);
      */
    } break;
    case U_SV_: {
      // kg44 this is not yet properly implemented...removed it
...
/// Resizes MULTI (GEM IPM work structure) data into internally scaled constant
/// mass
...
void TMulti::InitalizeGEM_IPM_Data() // Reset internal data formerly MultiInit()
{
  ...
  if (newInterval) { // build/rebuild internal lookup arrays
    node->MakeNodeStructures(window(), true, pm.Tai, pm.Pai);
  }

  // cout << "newInterval = " << newInterval << " pm.pTPD = " << pm.pTPD <<
  // endl;
...
//
  ...
  if (pm.pTPD < 2) {
    ...
    DC_LoadThermodynamicData(); // Loading thermodynamic data into MULTI
                                // structure
  }
  Alloc_internal();
  ...
  if (pm.pNP) { //  Smart Initial Approximation mode
    long int j, k;
    ...
    if (pm.FIs && AllPhasesPure == false) {
      ...
      int jb, je = 0;
      for (int kk = 0; kk < pm.FIs; kk++) { // loop on solution phases
        jb = je;
        je += pm.L1[kk];
        // Load activity coeffs for phases-solutions
        for (j = jb; j < je; j++) {
...
void TMulti::GEM_IPM_Init() {
  ...
  {
    for (j = 0; j < pm.L; j++) { // cleaning work vectors
  ...
  //   if( pmp->pKMM <= 0 )
  //   {
  //      KinMetModLoad();  // Call point to loading parameters for kinetic
  //      models pmp->pKMM = 1;
  //   }
  // #endif
  ...
  if (pm.FIs && AllPhasesPure == false) /// line must be tested !pm.FIs
  {
    ...
    if (pm.pIPN <= 0) // mixing models finalized in any case (AIA or SIA)
    {
      ...
      SolModLoad(); // Call point to loading scripts and parameters for mixing
                    // models
    }
    pm.pIPN = 1;
#endif
        ...
        for (int kk = 0; kk < pm.FIs; kk++) { // loop on solution phases
          ...
          if (pm.PHC[kk] == PH_POLYEL || pm.PHC[kk] == PH_SORPTION) {
            if (pm.PHC[0] == PH_AQUEL && pm.XF[kk] > pm.DSM &&
                (pm.XFA[0] > pm.ScMinM &&
                 pm.XF[0] > pm.XwMinM)) // fixed 30.08.2009 DK
              GouyChapman(
                  jb, je,
                  kk); // getting PSIs - electrical potentials on surface planes
          }
        }
      }
    }
    CalculateActivityCoefficients(LINK_TP_MODE);
    // Computing DQF, FugPure and G wherever necessary; Activity coeffs are
    // restored from lnGmo
...
void TMulti::Access_GEM_IMP_init() { GEM_IPM_Init(); }

TSolMod *TMulti::pTSolMod(int xPH) { return this->phSolMod[xPH]; }
...
void TMulti::DC_LoadThermodynamicData(TNode *aNa) // formerly CompG0Load()
{
  ...
  if (dCH->nTp < 1 || dCH->nPp < 1 || na->check_TP(TK, PPa) == false) {
    char buff[256];
    sprintf(buff,
            " Temperature %g or pressure %g out of range, or no T/D data are "
            "provided\n",
            TK, PPa);
  ...
  if (P < 1e-5) { // Pressure at saturated H2O vapour at given temperature
    long int xT = na->check_grid_T(TK);
    if (xT >= 0)
      P = dCH->Psat[xT] / bar_to_Pa;
    else
      P = LagranInterp(&PPa, dCH->TKval, dCH->Psat, PPa, TK, dCH->nTp, 1, 6) /
          bar_to_Pa;
  }
  pm.P = pm.Pc = P;
  ...
  for (k = 0; k < 5; k++) {
    ...
    if (xTP >= 0) {
      ...
      pm.denW[k] = LagranInterp(dCH->Pval, dCH->TKval, dCH->denW + jj, PPa, TK,
                                dCH->nTp, dCH->nPp, 6) /
                   1e3; // from test denW enough
      pm.epsW[k] = LagranInterp(dCH->Pval, dCH->TKval, dCH->epsW + jj, PPa, TK,
                                dCH->nTp, dCH->nPp, 5); // from test epsW enough
      pm.denWg[k] = LagranInterp(dCH->Pval, dCH->TKval, dCH->denWg + jj, PPa,
                                 TK, dCH->nTp, dCH->nPp, 5) /
                    1e3;
      pm.epsWg[k] = LagranInterp(dCH->Pval, dCH->TKval, dCH->epsWg + jj, PPa,
                                 TK, dCH->nTp, dCH->nPp, 5);
    }
  }
  ...
  for (k = 0; k < pm.FI; k++) {
    jb = je;
    je += pm.L1[k];
    // load t/d data from DC - to be extended for DCH->H0, DCH->S0, DCH->Cp0,
    // DCH->DD depending on the presence of these arrays in DATACH and Multi
    // structures
    for (j = jb; j < je; j++) {
      jj = j * na->gridTP();
      if (xTP >= 0) {
        ...
        Go = LagranInterp(dCH->Pval, dCH->TKval, dCH->G0 + jj, PPa, TK,
                          dCH->nTp, dCH->nPp, 6); // from test G0[Ca+2] enough
        Vv = LagranInterp(dCH->Pval, dCH->TKval, dCH->V0 + jj, PPa, TK,
                          dCH->nTp, dCH->nPp, 5) *
             1e5;
        if (dCH->S0)
          S0 = LagranInterp(dCH->Pval, dCH->TKval, dCH->S0 + jj, PPa, TK,
                            dCH->nTp, dCH->nPp, 4); // from test S0[Ca+2] enough
        if (dCH->H0)
          h0 = LagranInterp(dCH->Pval, dCH->TKval, dCH->H0 + jj, PPa, TK,
                            dCH->nTp, dCH->nPp, 5);
        if (dCH->Cp0)
          Cp0 = LagranInterp(dCH->Pval, dCH->TKval, dCH->Cp0 + jj, PPa, TK,
                             dCH->nTp, dCH->nPp,
                             3); // from test Cp0[Ca+2] not more
        if (dCH->A0)
          a0 = LagranInterp(dCH->Pval, dCH->TKval, dCH->A0 + jj, PPa, TK,
                            dCH->nTp, dCH->nPp, 5);
        if (dCH->U0)
          u0 = LagranInterp(dCH->Pval, dCH->TKval, dCH->U0 + jj, PPa, TK,
                            dCH->nTp, dCH->nPp, 5);
      }
#ifndef IPMGEMPLUGIN
      if (TSyst::sm->GetSY()
              ->Guns) // This is used mainly in UnSpace calculations
        Gg = TSyst::sm->GetSY()->Guns[pm.muj[j]]; // User-set increment to G0
                                                  // from project system
      // SDGEX     if( syp->GEX && syp->PGEX != S_OFF )   // User-set increment
      // to G0 from project system
      //            Ge = syp->GEX[pm.muj[j]];     //now Ge is integrated into
      //            pm.G0 (since 07.03.2008) DK
#else
      if (pm.tpp_G)
        pm.tpp_G[j] = Go;
      if (pm.Guns)
      ...
      Ge = 0.;
#endif
      pm.G0[j] = ConvertGj_toUniformStandardState(Go + Gg + Ge, j,
                                                  k); // formerly Cj_init_calc()
...
#endif
        switch (pm.PV) { // put molar volumes of components into A matrix or
                         // into the vector of molar volumes
        // to be checked!
        case VOL_CONSTR:
#ifndef IPMGEMPLUGIN
...
double TMulti::InternalEnergy(double TC, double P) {
  ...
  DC_LoadThermodynamicData(); // from lookup arrays
  GibbsEnergyMinimization();
  return ((pm.VXc - pm.VX_) +
          (pm.SXc - pm.SX_)); // VXc current value, VX_ value from key
}
