...
int checkargs(int argc, char **argv)
{
    ...
    while((opt_char = getopt_long(argc, argv, "i:t:h",
				      long_opts, &option_index)) != -1) {

        switch (opt_char) {
            case (int)'i': // -i or --input
                wellformed = true;
                iroot = optarg;
                RootName = iroot;
            ...
            case (int)'t': // -t or --type
                ...
                cout.flush();
                break;
            case (int)'h': // -h or --help
            case (int)'?': // Unrecognized option
            default:
                wellformed = false;
                break;
        }
    ...
    ****/
    ...
    for (int i = 0; i < names.size(); ++i) {
        ...
        if (writeXYZFile(times[i])) {
            cout << endl << "ERROR: Problem writing xyz file "
                 << i << endl << endl;
            return(1);
        }
    }
...
void getVcolors(vector<float> &red,
                vector<float> &green,
                vector<float> &blue)
{
...
void getTcolors(vector<float> &red,
                vector<float> &green,
                vector<float> &blue)
{
    for (int i = 0; i < NPHASES; ++i) {
...
void printHelp(void)
{
    ...
    cout << "       rootname is everything before .img in the file name" << endl << endl;
    return;
}
