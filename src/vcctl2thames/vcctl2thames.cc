...
{
    cout << endl;
    cout << "Usage: \"vcctl2thames [--verbose|-v] [--debug|-d] [--help|-h]\"" << endl;
...
int readVCCTLImage (string &input_filename,
                    int &xsize, int &ysize, int &zsize,
                    float &res, vector<int> &mic,
                    map<int,int> &idmap)
{
    ifstream fin;
    bool found = false;
    ...
    for (int i = 0; i < numsize; ++i) {
        ...
        fin >> sval;
        if (Verbose) cout << "Verbose: Read VCCTL value " << sval << endl;
        ...
        for (int j = 0; (j < vcctlid.size()) && (!found); ++j) {
            if (ival == vcctlid.at(j)) found = true;
        }
        if (!found) {
    ...
    cout << "*** VCCTL file read successfully, found "
         << vcctlid.size() << " unique ids" << endl << endl;
...
int readVCCTLHeader (ifstream &fin,
                    int &xsize, int &ysize,
                    int &zsize, float &res)
{
    string buff;
    ...
    if (buff.compare(VCCTL_Version_string) == 0) {
        fin >> ver;
        if (Verbose) cout << "Verbose:" << buff << ver << endl;
        fin >> buff;
        if (Verbose) cout << "Verbose:" << buff << endl;
        if (buff.compare(VCCTL_Xsize_string) == 0) {
            fin >> xsize;
            if (Verbose) cout << "Verbose:" << xsize << endl;
            fin >> buff >> ysize;
            if (Verbose) cout << "Verbose:" << buff << ysize << endl;
            fin >> buff >> zsize;
            if (Verbose) cout << "Verbose:" << buff << zsize << endl;
            fin >> buff >> res;
            if (Verbose) cout << "Verbose:" << buff << res << endl;
...
int getCorrespondences (vector<int> vcctlid,
                        map<int,int> &corr)
{
    corr.clear();
    int i = 0;
...
int writeTHAMESImage (string input_filename,
                      ...
                      map<int,int> idmap)
{
    ...
    cout << "*** Writing the THAMES image, will be called " << output_filename << endl;
...
int writeTHAMESHeader (ofstream &fout,
                       const int xsize, const int ysize,
                       const int zsize, const float res)
{
    ...
    fout << THAMES_ImgRes_string << " " << fixed << setprecision(4) << res << endl;
