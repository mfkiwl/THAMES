...
int main(int argc, char **argv) {
  ...
  if (errorProgram) {
    deleteDynAllocMem(ChemSys, Mic, RNG, ThermalStrainSolver,
                        AppliedStrainSolver, KController, Ctrl, starttime, lt, errorProgram);
  }
...
void deleteDynAllocMem(ChemicalSystem *ChemSys, Lattice *Mic, RanGen *RNG,
                         ...
                         Controller *Ctrl, clock_t st_time, time_t lt, bool errorProgram) {
  ...
  cout << endl << "=> IPM_dump.txt & ipmlog.txt are into " << outputFolder
       << " folder" << endl;
