...
:chemSys_(cs),solut_(solut),lattice_(lattice)
{
    ...
    blaine_ = kineticData.blaine;
    refBlaine_ = kineticData.refblaine;
    ...
    degreeOfHydration_ = kineticData.degreeOfHydration;
                                              ...
                                              bool isFirst)
{
    ...
    #ifdef DEBUG
        cout << "ParrotKillohModel::calculateKineticStep Hydration Time = "
             << hyd_time << endl;
        cout.flush();
    #endif
    ...
    try {
        ...
        int waterId = chemSys_->getDCId("H2O@");
        ...
        #ifdef DEBUG
            cout << "ParrotKillohModel::calculateKineticStep ICmoles before dissolving:" << endl;
            cout << "ParrotKillohModel::calculateKineticStep DC moles of water = " << chemSys_->getDCMoles(waterId);
            cout.flush();
            for (int i = 0; i < ICNum; i++) {
              ...
              cout << "ParrotKillohModel::calculateKineticStep     " << ICName[i] << ": " << ICMoles[i] << " mol" << endl;
            }
        #else 
            for (int i = 0; i < ICNum; i++) {
              ICMoles[i] = chemSys_->getICMoles(i);
        ...
        if (isFirst) {  // Beginning of special first-time setup tasks
            ...
            if (verbose_) {
                cout << "ParrotKillohModel::calculateKineticStep *** Initial solid mass = "
                     << solidMass << endl;
                cout << "ParrotKillohModel::calculateKineticStep *** w/s ratio = "
                     << getWcRatio() << endl;
                cout << "ParrotKillohModel::calculateKineticStep *** Initial water mass = "
                     << waterMass << endl;
                cout << "ParrotKillohModel::calculateKineticStep *** Initial water moles = "
                     << waterMoles << endl;
                cout << "ParrotKillohModel::calculateKineticStep ***" << endl;
                cout.flush();
            }
            ...
            double volume = 0.0;
            if (verbose_) {
                cout << "ParrotKillohModel::calculateKineticStep Initial MICROSTRUCTURE phase amount:" << endl;
                psMass = chemSys_->getMicroPhaseMass(microPhaseId_);
                psVolume = chemSys_->getMicroPhaseVolume(microPhaseId_);
                cout << "ParrotKillohModel::calculateKineticStep     " << chemSys_->getMicroPhaseName(microPhaseId_)
            ...
            for (int i = 0; i < ICMoles.size(); i++) {
                if (ICName[i] == "H") {
                    #ifdef DEBUG
                        cout << "ParrotKillohModel::calculateKineticStep Previous IC moles for H is: "
                             << ICMoles[i] << endl;
                        cout.flush();
                    #endif
                    ...
                    #ifdef DEBUG
                    if (verbose_) {
                        cout << "ParrotKillohModel::calculateKineticStep New ICmoles for H is: "
                             << ICMoles[i] << endl;
                        cout.flush();
                    }
                    #endif
                ...
                if (ICName[i] == "O") {
                    #ifdef DEBUG
                        cout << "ParrotKillohModel::calculateKineticStep Previous IC moles for O is: "
                             << ICMoles[i] << endl;
                        cout.flush();
                    #endif
                    ...
                    solut_->setICMoles(i,ICMoles[i]);
                    #ifdef DEBUG
                        cout << "ParrotKillohModel::calculateKineticStep New ICmoles for O is: "
                             << ICMoles[i] << endl;
                        cout.flush();
                    #endif
               }
        ...
        if (hyd_time < leachTime_ && hyd_time < sulfateAttackTime_) { 
          ...
          */
          ...
          if (DOH < 1.0 && !doTweak) {
              ...
              #ifdef DEBUG
                  cout << "ParrotKillohModel::calculateKineticStep Original scaled mass = " << initScaledMass_
                       << " and new scaled mass = "
          ...
          for (int ii = 0; ii < ICMoles.size(); ii++) {
              chemSys_->setICMoles(ii,ICMoles[ii]);
              #ifdef DEBUG
                  cout << "ParrotKillohModel::calculateKineticStep ICmoles of " << ICName[ii]
                       << " is: " << ICMoles[ii] << endl;
                  cout.flush();
              #endif
          }
