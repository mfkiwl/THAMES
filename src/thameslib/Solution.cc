...
Solution::Solution(const string &dchFileName, const string &dbrFileName,
                   const bool verbose) {
  pH_ = 7.0;
  pe_ = 1.0;
  Eh_ = 0.0;
  ...
  try {
    ...
    // if (jsonFormat_) {
    //     if (verbose) {
    //         cout << "Detected JSON input file format for Solution GEM data
    //         files" << endl; cout.flush();
    //     }
    ...
    // } else {
    //     if (verbose) {
    //         cout << "Detected key-value input file format for Solution GEM
    //         data files" << endl; cout.flush();
    //     }
  ...
  if (gemFlag == 1) {
    cout << "Bad return from GEM_init: " << dchFileName
  ...
  if (!(nodeStatus_ == OK_GEM_AIA || nodeStatus_ == OK_GEM_SIA)) {
    bool dothrow = false;
    cout << "Solution::Solution ERROR: Call to GEM_run in "
         << "Solution::Solution had an issue..." << endl;
  ...
  case 1: // JSON string is empty or corrupt
    cout << "Solution::Solution ERROR: Call to GEM_read_dbr "
         << "failed with nodeStatus_ = " << nodeStatus_
         << ", JSON string empty or corrupt" << endl;
    exit(1);
    break;
  case 2: // DBR is incompatibile with DCH/IPM
    cout << "Solution::Solution ERROR: Call to GEM_read_dbr "
         << "failed with nodeStatus_ = " << nodeStatus_
         << ", DBR incompatible with DCH/IPM" << endl;
    exit(1);
    break;
...
bool Solution::isInputFormatJSON(const char *masterFileName) {
  ...
  if (!in) {
    throw FileException("Solution", "isInputFormatJSON", masterFileName,
                        "Could not open");
  }
...
void Solution::getJSONFiles(const char *masterFileName, string &dchName,
                            string &ipmName, string &dbrName) {
  ifstream in(masterFileName);
  if (!in) {
    throw FileException("Solution", "getJSONFiles", masterFileName,
                        "Could not open");
  }
...
//  //if (cyc == 0)exit(0);
  ...
  if (!(nodeStatus_ == OK_GEM_AIA || nodeStatus_ == OK_GEM_SIA)) {
    bool dothrow = false;
    cout << "Solution::calculateState ERROR: Call to node::GEM_run "
         << "had an issue..." << endl;
    cout << "Solution::calculateState       nodeStatus_ = " << nodeStatus_;
    switch (nodeStatus_) {
    case NEED_GEM_AIA:
    ...
    case ERR_GEM_AIA:
      ...
      cout << msg << " and has failed " << timesGEMFailed_ << " in a row"
           << endl;
      timesGEMFailed_++;
...
double Solution::calculateCrystalStrain(double SI, double poreVolFrac,
                                        double Kp, double Ks) {
  crystalStrain_ = 0.0;
