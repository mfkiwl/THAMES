...
ChemicalSystem::ChemicalSystem(const string &GEMfilename,
  ...
  // if (verbose_) {
    cout << endl << "ChemicalSystem::Going into GEM_init (1) to read CSD file "
...
void ChemicalSystem::parseMicroPhases(const json::iterator cdi, int numEntries,
  ...
  for (int i = 0; i < (int)(cdi.value().size()); ++i) {
    ...
    bool done = false;
    for (int j = 0; j < (int)(phaseData.microPhaseDCPorosities.size()) && !done; ++j) {
...
int ChemicalSystem::calculateState(double time, bool isFirst = false,
  ...
  microVolume_ = 0.0;
  setPGEMPhaseStoich();   // call getPGEMPhaseStoich() => pGEMPhaseStoich_[i] // check!
                          // number of moles all ICs in all GEM CSD phases.
  setGEMPhaseStoich();    // call getGEMPhaseStoich() => GEMPhaseStoich_[i][j]  // check!
    ...
    if (!isKinetic_[i]) {
      ...
      for (int j = 0; j < microPhaseMembersSize_i; j++) {
        ...
        if (i == ELECTROLYTEID) {
          ...
          // cout << endl << "   i = " << i << "   j = " << j
          //      << "   GEMPhaseName_ = " << GEMPhaseName_[microPhaseMembers_[i][j]]
          //      << "   microPhaseVolume_[i](use GEMPhaseVolume_!!! ) = " << microPhaseVolume_[i]
          //      << "   numMoles(use waterMollarVol_(waterDCId)!!!) = "
          //      << microPhaseVolume_[i]/waterMollarVol_ <<  endl;
          // cout << endl << "   DCMoles_[waterDCId_] = " << DCMoles_[waterDCId_]
  ...
  if (isSaturated_) { // System is saturated
    ...
    //      << microPhaseVolume_[1] << "   waterMoles_ = " << DCMoles_[waterDCId_] << endl;
    // cout << endl << "ChemicalSystem::calculateState initMicroVolume_ = " << initMicroVolume_ << endl;
    // double waterDensity = waterMollarMass_ / waterMollarVol_ / 1.0e6; // g/cm3
    // double waterTotMass_0 = (microPhaseVolume_[1]/initMicroVolume_) * waterDensity * 100 / initSolidMass_;
    // double waterTotMoles_0 = waterTotMass_0 / waterMollarMass_; // mol
    // cout << endl << "ChemicalSystem::calculateState waterMolesCalc_M = " << waterTotMoles_0 << endl;
    // cout << endl << "ChemicalSystem::calculateState waterMolesCalc_V = " <<
...
void ChemicalSystem::setMicroPhaseSI(double time) {
  ...
  try {
    ...
    if (time >= iniAttackTime_) {
      cout << endl << "          SI calc from GEM phases for mPhId = 15 - mPhName = "
    ...
    for (int i = FIRST_SOLID; i < numMicroPhases_; ++i) {
      pname = microPhaseName_[i];
      aveSI = totMoles = 0.0;
      microPhasePhMembers = microPhaseMembers_[i]; //getMicroPhaseMembers(i); //microPhaseMembers_ vs GEMPhaseIdLookup_
      ...
      if ((time >= iniAttackTime_) && (i == 15)) {
        for (int ii = 0; ii < sizeMicroPhasePhMembers; ++ii) {
          ...
          cout << "           ii/newPhId/moles/Ph_SatInd/SI/newPhName = "
               << ii << " / " << newPhId << " / " << moles << " / "
               << node_->Ph_SatInd(newPhId) << " / "
               << pow(10, node_->Ph_SatInd(newPhId)) << " / " << GEMPhaseName_[newPhId] << endl;
        }
      }
      for (int ii = 0; ii < sizeMicroPhasePhMembers; ++ii) {
        newPhId = microPhaseMembers_[i][ii]; // microPhasePhMembers[i];
    ...
    // if (time >= iniAttackTime_) {
    //   setSI();
    //   cout << endl << "          SI calc from GEM DCs for mPhId = 15 - mPhName = "
    //        << microPhaseName_[15] << " : " << endl;
    // }
...
void ChemicalSystem::initElasticModuliMap(void) {
  // (1): Table 1 from Haecker et al. / Cement and Concrete Research 35(2005)1948–1960
  ...
  elasticModuli_["Calcite"].altName = "Cc, CaCO3, Limestone, Aragonite, Vaterite";
  ...
  elasticModuli_["AFmc"].altName = "C4AcH11, AFm-c, Monoarbonate, Carboaluminate";
...
void ChemicalSystem::writeSatElectrolyteGasConditions(void) {
  ...
  if (initialSolutionComposition_.size() > 0) initial = true;
  if (fixedSolutionComposition_.size() > 0) fixed = true;

  if (initial || fixed) {
    if (initial && fixed) {
      cout << endl << "error : check the chemistry.json file!" << endl;
      cout << endl << "if \"electrolyte_conditions\" is present, \"condition\" "
                      "can be only one of the next combinations:" << endl;
      cout << "\"initial\"" << endl;
      cout << "\"fixed\"" << endl;
      cout << "\"attack\"" << endl;
      ...
      cout << "   - hydration starts with a given electrolyte composition :" << endl;
      ...
      cout << "   - during hydration the electrolyte has a fixed composition :" << endl;
    ...
    cout << "   - hydration starts without any dissolved ionic species (pure water)" << endl;
  }
  if (attackSolutionComposition_.size() > 0) {
    cout << "   - after hydration, simulation continues with an attack - solution composition being :" << endl;
  ...
  if (attackSolutionComposition_.size() > 0) {
    cout << "   - simulation continues with attack, solution composition being :" << endl;
    cout << "      electrolyte - attack" << endl;
    map<int, double>::iterator it = attackSolutionComposition_.begin();
    while (it != attackSolutionComposition_.end()) {
      DCId = it->first;
      if (DCId != waterDCId_) {
  ...
  */
  ...
  initial = false;
  fixed = false;
  if (initialGasComposition_.size() > 0) initial = true;
  if (fixedGasComposition_.size() > 0) fixed = true;

  if (initial || fixed) {
    if (initial && fixed) {
      cout << endl << "error : check the chemistry.json file!" << endl;
      cout << endl << "if \"gas_conditions\" is present, \"condition\" "
                      "can be only one of the next combinations:" << endl;
      cout << "\"initial\"" << endl;
      cout << "\"fixed\"" << endl;
      cout << "\"attack\"" << endl;
      ...
      cout << "   - hydration starts in an atmosphere having an initial gas composition :" << endl;
      ...
      cout << "   - during hydration the atmosphere has a fixed gas composition :" << endl;
      cout << "      gas - fixed" << endl;
  ...
  if (attackGasComposition_.size() > 0) {
    cout << "   - after hydration, simulation continues with gas attack - gas composition being :" << endl;
    cout << "      condition - attack" << endl;
  ...
  if (attackSolutionComposition_.size() == 0 &&
    ...
    cout << "   - after hydration, no attack (electyrolyte/gas) takes place" << endl;
  }
...
void ChemicalSystem::setElectrolyteComposition(const bool isFirst,
  ...
  // cout << "  ----> ChemicalSystem::setElectrolyteComposition - doAttack/waterMass : "
  //      << doAttack << " / " << waterMass << endl;

  if (doAttack) {
    cout << endl <<  "  ----> ChemicalSystem::setElectrolyteComposition - doAttack/waterMass : "
         << doAttack << " / " << waterMass << endl;

    // cout << endl <<  "           initial - DCId/DCMoles/DCName : " << endl;
    // for(int i = 0; i < numDCs_; i++){
    //   // if (i < 68) DCMoles_[i] = 0;
    //   cout << "                     " << i
    ...
    /*
    int Ca_DCId = getDCId("Ca+2");
    int Na_DCId = getDCId("Na+");
    int K_DCId = getDCId("K+");
    int SO4_DCId = getDCId("SO4-2");
    int OH_DCId = getDCId("OH-");
    double targetConc = 0.0005;
    double targetMoles = targetConc * waterMass;

    double iniElectrCharge = 2 * DCMoles_[Ca_DCId] + DCMoles_[Na_DCId] +
                             DCMoles_[K_DCId] - 2 * DCMoles_[SO4_DCId];

    cout << "  ----> ChemicalSystem::setElectrolyteComposition - LEACHING ini:" << endl;
    cout << "        Ca_DCId  = " << Ca_DCId   << "  ->  DCMoles_ = " << DCMoles_[Ca_DCId] << endl;
    cout << "        Na_DCId  = " << Na_DCId   << "  ->  DCMoles_ = " << DCMoles_[Na_DCId] << endl;
    cout << "        K_DCId   = " << K_DCId    << "  ->  DCMoles_ = " << DCMoles_[K_DCId] << endl;
    cout << "        SO4_DCId = " << SO4_DCId  << "  ->  DCMoles_ = " << DCMoles_[SO4_DCId] << endl;
    cout << "              iniElectrCharge = " << iniElectrCharge << endl;
    cout << "        OH_DCId  = " << OH_DCId   << "  ->  DCMoles_ = " << DCMoles_[OH_DCId] << endl;

    if (DCMoles_[Ca_DCId] > 0.0006 * waterMass) {
      DCMoles_[Ca_DCId] = 1.e-7 * waterMass;
    } else {
      DCMoles_[Ca_DCId] = targetMoles;
    }

    if (DCMoles_[Na_DCId] * 0.6 > targetMoles) {
      DCMoles_[Na_DCId] = 0.6 * targetMoles;
    } else {
      DCMoles_[Na_DCId] = targetMoles;
    }

    if (DCMoles_[K_DCId] * 0.6 > targetMoles) {
      DCMoles_[K_DCId] = 0.6 * targetMoles;
    } else {
      DCMoles_[K_DCId] = targetMoles;
    }

    if (DCMoles_[SO4_DCId] > targetMoles)
      DCMoles_[SO4_DCId] = targetMoles;

    double finElectrCharge = 2 * DCMoles_[Ca_DCId] + DCMoles_[Na_DCId] +
                             DCMoles_[K_DCId] - 2 * DCMoles_[SO4_DCId];

    DCMoles_[OH_DCId] = DCMoles_[OH_DCId] - iniElectrCharge + finElectrCharge;

    cout << "  ----> ChemicalSystem::setElectrolyteComposition - LEACHING fin:" << endl;
    cout << "        Ca_DCId  = " << Ca_DCId   << "  ->  DCMoles_ = " << DCMoles_[Ca_DCId] << endl;
    cout << "        Na_DCId  = " << Na_DCId   << "  ->  DCMoles_ = " << DCMoles_[Na_DCId] << endl;
    cout << "        K_DCId   = " << K_DCId    << "  ->  DCMoles_ = " << DCMoles_[K_DCId] << endl;
    cout << "        SO4_DCId = " << SO4_DCId  << "  ->  DCMoles_ = " << DCMoles_[SO4_DCId] << endl;
    cout << "              finElectrCharge = " << finElectrCharge << endl;
    cout << "              netElectrCharge = " << iniElectrCharge - finElectrCharge << endl;
    cout << "        OH_DCId  = " << OH_DCId   << "  ->  DCMoles_ = " << DCMoles_[OH_DCId] << endl;
    */
    ...
    if (attackSolutionComposition_.size() > 0) {
      ...
      while (it != attackSolutionComposition_.end()) {
        ...
        cout << "    ----> ChemicalSystem::setElectrolyteComposition attack - waterMass/DCId/DCconc/DCMoles_ : "
             << waterMass << " / "  << DCId << " / " << DCconc << " / " << DCMoles_[DCId] << endl;
        it++;       
      }
      // } else {
      //   cout << endl << "ChemicalSystem::setElectrolyteComposition() error for cyc = "
      //        << cyc << "  : " << endl;
      //   cout << "doAttack is true but atackSolutionComposition_.size() = "
      //        << atackSolutionComposition_.size() << endl;
      //   cout << endl << "stop program!" << endl;
      //   exit(0);
    }

    // cout << endl <<  "           final - DCId/DCMoles/DCName : " << endl;
    // for(int i = 0; i < numDCs_; i++){
    //   cout << "                     " << i
    //        << " / " << DCMoles_[i] << " / " << DCName_[i] << endl;
    // }
