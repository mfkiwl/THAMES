...
*/
  ...
  for (i = 0; (i < time_.size()) && (capwater);
    ...
    if (chemSys_->getTimesGEMFailed() > 0) {
      ...
      if (i == 0) {
        cout << endl
             << "Controller::doCycle  GEMFailed   i/cyc/time_[i]/timestep) : "
             << i << " / " << cyc << " / " << time_[i] << " / " << timestep
             << endl;
      } else {
        cout << endl
             << "Controller::doCycle  normal   i/cyc/time_[i]/time_[i - "
                "1]/timestep) : "
    ...
    for (i = 0; (i < timeSize) && (capwater); ++i) { //main computation cycle loop
            ...
            */
            ...
            if (changeLattice == 0) {
                ...
                while (changeLattice == 0){ //} // - for many phases or more loops for the same phase!
                    ...
                    lattice_->resetRNG(iniLattice.latticeRNGseed, iniLattice.numRNGcall_0,
      ...
      */
      ...
      //
      // restore initial system:
      // - for many phases or more loops for the same phase!
      // if (changeLattice == 0) {
      while (changeLattice == 0) {
        //} // - for many phases or more loops for the same phase!
        // if (changeLattice == 0) {
        DCId = chemSys_->getMicroPhaseDCMembers(phDiff, 0);
