...
Lattice::Lattice(ChemicalSystem *cs, RanGen *rg, int seedRNG,
  ...
  double vfrac, mfrac, capfrac, surfArea, molarMass, molarVolume, density;
...
int Lattice::emptyPorosity(int numsites, int cyc) {
  ...
  ///
  /// @todo Consider removing some of the standard output, or setting a flag
  /// for it.
  ///
...
int Lattice::changeMicrostructure(double time, const int simtype,
  ...
  // cout << endl << "  Lattice::changeMicrostructure -ini- cyc = " << cyc <<
  // endl;
  ...
  ///
  /// @todo This function is very large; consider breaking it into small
  /// pieces for ease of maintenance and readability.
  ///
...
void Lattice::writeLattice(double curtime, const int simtype,
  ...
  if (simtype == SULFATE_ATTACK) {
    ...
    out1.close();
  } // The above block is implemented only if we are dealing with sulfate
    // attack
}
...
void Lattice::findIsolatedClusters(void) {
  ...
  // vfrac we use canDissolve vector instead count_ vector
