...
#ifdef DEBUG
  cout << "PozzolanicModel::calculateKineticStep Hydration Time = " << hyd_time
       << endl;
  cout.flush();
#endif
    ...
    if (hyd_time < leachTime_ && hyd_time < sulfateAttackTime_) {
      ...
      if (DOR < 1.0) {
        ...
        if (fabs(dissolutionRateConst_) > 0.0) {
          ...
          double kna =
              6.35e-7; // mol m-2 s-1 ads. rate const from Dove and Crerar
          double Kna =
              58.3; // adsorption equilibrium constant from Dove and Crerar
          double k = chemSys_->getDCConcentration("K+");
          double kk =
              5.6e-7; // mol m-2 s-1 ads. rate const from Dove and Crerar
          double Kk =
              46.6; // adsorption equilibrium constant from Dove and Crerar
          ...
          int GEMPhaseIndex =
              chemSys_->getMicroPhaseToGEMPhase(microPhaseId_, 0);
        ...
        if (DOR > 0.0) {
          ...
          //          * ssaFactor_ * pow((1.0 -
          //          pow(saturationIndex,siexp_)),dfexp_)
        ...
        cout << "    (baserateconst = " << diffusionRateConstLate_ << ")"
             << endl;
        cout << "    (dor = " << DOR << ")" << endl;
        cout << "  diffrate = " << diffrate << endl;
        cout << "  RATE = " << rate << endl;
        cout << "  timestep " << timestep << endl;
        cout << "  oldDOR = " << DOR << ", new DOR = " << newDOR << endl;
        cout.flush();
...
#ifdef DEBUG
      cout << "PozzolanicModel::calculateKineticStep ICmoles after dissolving:"
           << endl;
      for (int i = 0; i < ICNum; i++) {
